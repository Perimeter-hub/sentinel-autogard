...
const USER_AGENT = "Sentinel Autogard Full Site Audit/0.1";

async function discover(query) {
  const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=5&countrycodes=ch&q=${encodeURIComponent(query)}`, { headers: { "User-Agent": USER_AGENT, Accept: "application/json" }, cache: "no-store" });
...
export async function POST(request) {
  try {
    const { query, radius = 1000 } = await request.json();
    if (!query?.trim()) return NextResponse.json({ error: "query is required" }, { status: 400 });

    const candidates = await discover(query.trim());
    ...
    const candidateBoundary = selected.boundingbox ? {
      south: Number(selected.boundingbox[0]),
      north: Number(selected.boundingbox[1]),
      west: Number(selected.boundingbox[2]),
      east: Number(selected.boundingbox[3]),
      confidence: selected.type === "industrial" || selected.type === "commercial" ? "medium" : "low",
      state: "review_required"
    } : null;

    return NextResponse.json({ data: {
      status: "site_discovered",
      site: { displayName: selected.display_name, latitude, longitude, osmType: selected.type, osmId: selected.osm_id, address: selected.address || {} },
      candidateBoundary,
      features: classified,
      summary,
      nextActions: ["Review candidate site", "Accept or edit boundary", "Run visual evidence analysis", "Review security observations"]
    }});
  } catch (error) {
