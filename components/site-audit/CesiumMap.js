...
const CesiumMap = forwardRef(function CesiumMap({ onMetricsChange, onStatusChange, onGeometryChange, onLocationSelected, onBoundaryDetected }, ref) {
  ...
  useEffect(() => {
    ...
    async function initializeCesium() {
      ...
      const viewer = new Cesium.Viewer(containerRef.current, {
        ...
        fullscreenButton: true, terrainProvider: new Cesium.EllipsoidTerrainProvider()
      });
  ...
  function renderBoundary(geometry, options = {}) {
    ...
    const entity = viewer.entities.add({
      polygon: {
        ...
        material: (options.color || Cesium.Color.YELLOW).withAlpha(options.alpha ?? 0.16),
        outline: true,
        outlineColor: options.color || Cesium.Color.YELLOW,
  ...
  async function locate(query) {
    ...
    if (coordinateMatch) {
      latitude = Number(coordinateMatch[1]); longitude = Number(coordinateMatch[2]);
    } else {
      try {
    ...
    const location = { latitude, longitude, displayName, boundingBox };
  ...
  function enableMapSelection() {
    ...
    handler.setInputAction((movement) => {
      ...
      const marker = viewer.entities.add({ position, point: { pixelSize: 11, color: Cesium.Color.CYAN, outlineColor: Cesium.Color.WHITE, outlineWidth: 2 } });
  ...
  function startPolygonDrawing() {
    ...
    const dynamicPolygon = viewer.entities.add({ polygon: {
      hierarchy: new Cesium.CallbackProperty(() => positions.length >= 3 ? new Cesium.PolygonHierarchy(positions) : undefined, false),
      material: Cesium.Color.CYAN.withAlpha(0.25), outline: true, outlineColor: Cesium.Color.CYAN
    }});
